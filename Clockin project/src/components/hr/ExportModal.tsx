...
export function ExportModal({ open, onClose, records, employeeById, range, filterSummary }: ExportModalProps) {
  const handleExport = () => {
    downloadCsv(`cadence-attendance-${range.from}_${range.to}.csv`, buildCsv(records, employeeById));
    onClose();
    toast.success(`Exported ${records.length} records`, { description: 'cadence-attendance.csv downloaded' });
  };
