...
export function Reports() {
  const { employeeById, employees, departments } = useApp();
  const [exporting, setExporting] = useState(false);

  const today = new Date();
  const start = new Date(today);
  start.setDate(today.getDate() - 29);
  const { records: attendanceHistory } = useHrRecords(undefined, isoDate(start), isoDate(today));
  ...
  }, [employees, departments, attendanceHistory]);
